...
static const struct emif_regs ddr3_smarc80_emif_regs_400Mhz = {
        .sdram_config                   = 0x63841372,
        .sdram_config2                  = 0x00000000,
        .ref_ctrl                       = 0x00000c30,
        .sdram_tim1                     = 0xeaaad4db,
        .sdram_tim2                     = 0x266b7fda,
...
static const struct emif_regs ddr3_smarc1g_emif_regs_400Mhz = {
        .sdram_config                   = 0x638413b2,
        .sdram_config2                  = 0x00000000,
        .ref_ctrl                       = 0x00000c30,
        .sdram_tim1                     = 0xeaaad4db,
...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	else if (board_is_gpevm() || board_is_sk() || board_is_smarc_t437x_800() || board_is_smarc_t437x_01g())
...
enum {
	...
	RTC_BOARD_GPEVM,
	RTC_BOARD_SK,
	RTC_BOARD_SMARC_T437X_800,
	RTC_BOARD_SMARC_T437X_01G,
};
...
void rtc_only_update_board_type(u32 btype)
{
	...
	case RTC_BOARD_SK:
		name = "AM43__SK";
		break;
        case RTC_BOARD_SMARC_T437X_800:
                name = "SMCT4X80";
                break;
        case RTC_BOARD_SMARC_T437X_01G:
                name = "SMCT4X1G";
                break;
	}
...
u32 rtc_only_get_board_type(void)
{
	if (board_is_eposevm())
		...
		return RTC_BOARD_SK;
        else if (board_is_smarc_t437x_800())
                return RTC_BOARD_SMARC_T437X_800;
        else if (board_is_smarc_t437x_01g())
                return RTC_BOARD_SMARC_T437X_01G;
	...
	if (board_is_eposevm()) {
		...
		config_ddr(400, &ioregs_ddr3, NULL, NULL,
			   &ddr3_idk_emif_regs_400Mhz, 0);
                ...
                config_ddr(400, &ioregs_ddr3, NULL, NULL,
                           &ddr3_smarc80_emif_regs_400Mhz, 0);
        } else if (board_is_smarc_t437x_01g()) {
                config_ddr(400, &ioregs_ddr3, NULL, NULL,
                           &ddr3_smarc1g_emif_regs_400Mhz, 0);
	}
}
#endif
...
#define GPIO_BOOT_SEL1		168
#define GPIO_BOOT_SEL2          169
#define GPIO_BOOT_SEL3          170
        ...
        gpio_request(GPIO_BOOT_SEL1, "boot_sel1");
        gpio_direction_input(GPIO_BOOT_SEL1);
        gpio_request(GPIO_BOOT_SEL2, "boot_sel2");
        gpio_direction_input(GPIO_BOOT_SEL2);
        gpio_request(GPIO_BOOT_SEL3, "boot_sel3");
        gpio_direction_input(GPIO_BOOT_SEL3);
	...
	/*Read BOOT_SEL Configuration */
        if ((gpio_get_value(168) == 0)&&(gpio_get_value(169) == 0)&&(gpio_get_value(170) == 0)) {
                ...
                setenv("bootcmd", "dhcp;");
        } else if ((gpio_get_value(168) == 1)&&(gpio_get_value(169) == 1)&&(gpio_get_value(170) == 0)) {
                puts("BOOT_SEL Detected: ON ON OFF, Carrier SPI Boot Not Supported...\n");
                hang();
        } else if ((gpio_get_value(168) == 0)&&(gpio_get_value(169) == 0)&&(gpio_get_value(170) == 1)) {
                puts("BOOT_SEL Detected: OFF OFF ON, Load zImage from USB1...\n");
                setenv_ulong("usbdev", 0);
                setenv("bootcmd", "run findfdt; run usbboot;");
        } else if ((gpio_get_value(168) == 1)&&(gpio_get_value(169) == 1)&&(gpio_get_value(170) == 1)) {
                puts("BOOT_SEL Detected: ON ON ON, MOdule SPI Boot up is Default, Load zImage from Module eMMC...\n");
                setenv_ulong("mmcdev", 1);
                setenv("bootcmd", "run findfdt; run mmcboot;");
        } else {
                puts("unsupported boot up devices\n");
                hang();
        }
	return 0;
}
...
{
        if (board_is_gpevm() && !strcmp(am43xx_board_name, "AM43__GP"))
                return 0;
        else if (board_is_sk() && !strcmp(am43xx_board_name, "AM43__SK"))
                return 0;
        else if (board_is_eposevm() && !strcmp(am43xx_board_name,"AM43EPOS"))
                return 0;
        else if (board_is_idk() && !strcmp(am43xx_board_name, "AM43_IDK"))
                return 0;
        else if (board_is_smarc_t437x_800() && !strcmp(am43xx_board_name, "SMCT4X80"))
                return 0;
        else if (board_is_smarc_t437x_01g() && !strcmp(am43xx_board_name, "SMCT4X1G"))
                return 0;
        else
                return -1;
}
