...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	else if (board_is_evm() || board_is_sk() || board_is_smarc_t437x_800() || board_is_smarc_t437x_01g())
...
{
        struct ti_am_eeprom data;

        if (board_is_smarc_t437x_800() || board_is_smarc_t437x_01g()) {

        /* Read Board Info from EEPROM */
        puts("-----------------------------------------\n");
        printf("Board ID:               %.*s\n",
               sizeof(data.name), board_ti_get_name());
        printf("Board Revision:         %.*s\n",
               sizeof(data.version), board_ti_get_rev());
        printf("Board Serial#:          %.*s\n",
               sizeof(data.serial), board_ti_get_serial());
        puts("-----------------------------------------\n");

        } else {
        puts("Bad EEPROM or unknown board!\n");
                return 0;
        }

        /* LCD and Backlight Enable Pin */
