...
static inline int board_is_fimx6qp1g(struct smarcfimx6_id *header)
{
        return !strncmp(header->name, "SMCMX1QP", HDR_NAME_LEN);
}

static inline int board_is_fimx6qp2g(struct smarcfimx6_id *header)
{
        return !strncmp(header->name, "SMCMX2QP", HDR_NAME_LEN);
}
