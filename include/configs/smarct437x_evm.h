...
#ifndef __CONFIG_SMARCT437X_EVM_H
#define __CONFIG_SMARCT437X_EVM_H
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"usbargs=setenv bootargs console=${console} " \
        ...
        "loadusbbootenv=load usb ${bootpart} ${loadaddr} ${bootenv}\0" \
        "importbootenv=echo Importing environment from mmc ...; " \
                "env import -t -r $loadaddr $filesize\0" \
        "importusbbootenv=echo Importing environment from usb ...; " \
                "env import -t -r $loadaddr $filesize\0" \
	...
	"loadramdisk=load ${devtype} ${devnum} ${rdaddr} ramdisk.gz\0" \
        ...
        "loadfdt=echo loading ${fdtdir}/${fdtfile} ...; load ${devtype} ${mmcdev}:1 ${fdtaddr} ${bootdir}/${fdtfile}\0" \
	...
	"findfdt="\
		...
		"if test $board_name = AM43_IDK; then " \
			"setenv fdtfile am437x-idk-evm.dtb; fi; " \
                "if test $board_name = SMCT4X80; then " \
                        "setenv fdtfile am437x-smarct437x.dtb; fi; " \
                "if test $board_name = SMCT4X1G; then " \
                        "setenv fdtfile am437x-smarct437x.dtb; fi; " \
...
#endif	/* __CONFIG_SMARCT437X_EVM_H */
