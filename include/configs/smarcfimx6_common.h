				...
				"if test $board_name = SABRESD && test $board_rev = MX6DL; then " \
					"setenv fdt_file imx6dl-sabresd.dtb; fi; " \
                               	"if test $board_name = SMCMX1QP || test $board_name = SMCMX2QP && test $board_rev = MX6QP; then " \
                                       	"setenv fdt_file imx6qp-smarcfimx6.dtb; fi; " \
                               	"if test $board_name = SMCMXQ1G || test $board_name = SMCMXQ2G || test $board_name = SMCMXD1G || test $board_name = SMCMXD2G && test $board_rev = MX6Q; then " \
                                       	"setenv fdt_file imx6q-smarcfimx6.dtb; fi; " \
                               	"if test $board_name = SMCMXU1G || test $board_name = SMCMXSLO && test $board_rev = MX6DL; then " \
                                       	"setenv fdt_file imx6dl-smarcfimx6.dtb; fi; " \
				"if test $fdt_file = undefined; then " \
					"echo WARNING: Could not determine dtb to use; " \
				"fi; " \
			"fi;\0" \
...
#include "smarcfimx6android_common.h"
#else
